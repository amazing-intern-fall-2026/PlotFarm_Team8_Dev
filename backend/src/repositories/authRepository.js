  ...
  if (custByEmail) {
    ...
    if (linkedAcc) {
      return {
        ...
        email: custByEmail.Email,
        fullName: custByEmail.TenKH,
...
/** Find latest valid reset OTP record for username */
...
export const markOtpAsUsed = async (id, transaction) => {
  const request = new sql.Request(transaction || getPool());
  const query = `UPDATE dbo.DATLAIMATKHAU SET IsUsed = 1 WHERE Id = @id`;
...
export const updateAccountPassword = async (username, newPasswordHash, transaction) => {
  const request = new sql.Request(transaction || getPool());
  const query = `UPDATE dbo.TAIKHOAN SET MatKhauHash = @hash WHERE TenDangNhap = @username`;
  request.input('username', sql.VarChar, username);
  request.input('hash', sql.VarChar, newPasswordHash);
  const result = await request.query(query);
